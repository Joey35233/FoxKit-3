...
cbuffer UnityPatch_SScene
{
    ...
    float4 UnityPatch_g_sScene_m_fogParam[3]; ///< フォグパラメータ
...
{
    float4 UnityPatch_g_sLights_m_lightParams[SHADER_CONSTANTBUFFER_LIGHTS_SIZE];
};
...
cbuffer UnityPatch_SObject
{
    ...
    float4 UnityPatch_g_sObject_m_localParam[4]; ///< ローカルパラメータ(各レンダリングフェーズのローカル処理で利用)
};
