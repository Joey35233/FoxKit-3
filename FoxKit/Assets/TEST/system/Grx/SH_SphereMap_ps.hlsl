...
#line 10 "..\Gr\Dg\shader\SH_SphereMap_Common.shdr"

struct PSParamSH {
	TMatrix4x4  m_matrix[4];
};
struct PSParamSHSky {
	TMatrix4x4  m_matrix[3];
};
struct PSParamIdParam {
	float4 m_param;
};

cbuffer PsWork	: PS_REGISTER(WORK)
{
	static PSParamSH 		g_psParamSH;
	static PSParamSHSky 	g_psParamSHSky;
	static PSParamIdParam	g_psParamId;
}

#line 34 "..\Gr\Dg\shader\SH_SphereMap_Common.shdr"
...
#line 68 "..\Gr\Dg\shader\SH_SphereMap_Common.shdr"
inline void NDrawSphericalMap(float2 inClip, out half4 outRGB)
{
#line 73 "..\Gr\Dg\shader\SH_SphereMap_Common.shdr"
	...
	float3 worldNormal = ApplyMatrixT(g_psScene.m_view, float4(viewNormal, 0)).xyz;
	...
	half occlusion;
	outRGB.r = (half)dot(cartesian * exposure, ApplyMatrixT(g_psParamSH.m_matrix[0], cartesian));
	outRGB.g = (half)dot(cartesian * exposure, ApplyMatrixT(g_psParamSH.m_matrix[1], cartesian));
	outRGB.b = (half)dot(cartesian * exposure, ApplyMatrixT(g_psParamSH.m_matrix[2], cartesian));
	occlusion = (half)dot(cartesian, ApplyMatrixT(g_psParamSH.m_matrix[3], cartesian));
	half3 skylight;
	skylight.r = (half)dot(cartesian * exposure, ApplyMatrixT(g_psParamSHSky.m_matrix[0], cartesian));
	skylight.g = (half)dot(cartesian * exposure, ApplyMatrixT(g_psParamSHSky.m_matrix[1], cartesian));
	skylight.b = (half)dot(cartesian * exposure, ApplyMatrixT(g_psParamSHSky.m_matrix[2], cartesian));
...
#line 14 "..\Gr\Dg\shader\SH_SphereMap.shdr"
inline void NEncodeHDR(half4 inRGB, out half4 outColor)
{
#line 20 "..\Gr\Dg\shader\SH_SphereMap.shdr"



	/*
	//SHMaps作成時のエンコード　露出補正＊ライトスケール（通常四分の一）を掛ける
	//Win版はその後、ライティングもすべて四分の一で行なってDeferred完了時に４倍して出力している
	//X360、PS3はSHMaps描画時に四分の一してHDRに書きだす際に４倍するようにするベキ
		outColor.rgba = half4( inRGB.rgb * g_psScene.m_exposure.x, inRGB.w ) ;
	*/
#ifdef F_TARGET_PS3
	//PS3版だけは書く時に加工が必要になっている
	//SRGBWriteでブレンド出来ない（リニアに変換してブレンドしれくれない）ので二乗で出力してネガを軽減してみる
	inRGB.xyz = pow(inRGB.xyz, 0.7f);
#endif
...
#line 61 "..\Gr\Dg\shader\SH_SphereMap.shdr"
void ps_main(
...
#line 73 "..\Gr\Dg\shader\SH_SphereMap.shdr"
	NDrawSphericalMap_drawSphericalMap_inClip = inTexcoord.xy;
	NDrawSphericalMap(NDrawSphericalMap_drawSphericalMap_inClip, NDrawSphericalMap_drawSphericalMap_outRGB);


	half4	NEncodeHDR_encodeHDR_inRGB;
	half4 NEncodeHDR_encodeHDR_outColor;
#line 74 "..\Gr\Dg\shader\SH_SphereMap.shdr"
	NEncodeHDR_encodeHDR_inRGB = NDrawSphericalMap_drawSphericalMap_outRGB;
	NEncodeHDR(NEncodeHDR_encodeHDR_inRGB, NEncodeHDR_encodeHDR_outColor);


#line 76 "..\Gr\Dg\shader\SH_SphereMap.shdr"
	outColor = NEncodeHDR_encodeHDR_outColor;
