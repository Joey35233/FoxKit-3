...
#line 76 "..\Gr\Dg\shader\SSAOLineIntegrals.shdr"
inline void NVolumetricOcclusion( float2 inTexcoord, float2 inPixelPosition, Texture2D inLinearDepth, Texture2D inRotations, out half4 outColor)
{
#line 84 "..\Gr\Dg\shader\SSAOLineIntegrals.shdr"
	...
	#define NUM_PAIRS (3)
	const float2 sphereSamplePoints[NUM_PAIRS] = {
		float2( -0.125000, 0.216506 ),
		float2( -0.250000, -0.433013 ),
		float2( 0.750000, 0.000000 )
};

	
	const float invSphereLineLengths[NUM_PAIRS] = {1.0f/1.936208f, 1.0f/1.731469f, 1.0f/1.322876f};

	
	const half sphereSampleWeights[NUM_PAIRS] = {0.156523, 0.176937, 0.132698};
	const half sphereCenterWeight = 0.067685 ;

#if 0
	const half2 paramRadius = g_psObject.m_localParam[0].xy ;
	const half2 paramMaxDistance = g_psObject.m_localParam[0].zw ;
	const half2 paramFalloffDistance  = g_psObject.m_localParam[1].xy ;
#else
	const half2 paramRadius = (half2)g_psObject.m_localParam[0].xy ; // J - LineSSAOParameters.innerRadius, LineSSAOParameters.outerRadius
	const half2 paramMaxDistance = (half2)g_psObject.m_localParam[1].xy ; // J - LineSSAOParameters.maxDistanceInner, LineSSAOParameters.maxDistanceOuter
	const half2 paramInvMaxDistance = (half2)g_psObject.m_localParam[2].xy ; // J - From every one of many samples: 2/3, 1/3
	const half2 paramFalloffDistance  = (half2)g_psObject.m_localParam[3].xy ; // J - float2(-(LineSSAOParameters.falloffStart + LineSSAOParameters.falloffRange), -1/LineSSAOParameters.falloffRange)
	const half2 paramFallonDistance  = (half2)g_psObject.m_localParam[0].zw ; // J - float2(-(LineSSAOParameters.gainonStartOverride <= 0 ? 0x3d8d4fdf : LineSSAOParameters.gainonStartOverride), 1/LineSSAOParameters.gainonRange)
#endif

	half radii[NUM_RADII] = {paramRadius.x, paramRadius.y};
	half maxDistances[NUM_RADII] = {paramMaxDistance.x, paramMaxDistance.y};
	half invMaxDistances[NUM_RADII] = {paramInvMaxDistance.x, paramInvMaxDistance.y};
#else	// ifndef FAST_SSAO
	/* ---------------------------------------------------------------- */
	// サンプル数を大幅に減らした高速化版
	/* ---------------------------------------------------------------- */
	...
	#define NUM_PAIRS (2)
	const float2 sphereSamplePoints[NUM_PAIRS] = {
		float2( 0.30, 0.0 ),// length:0.3 deg:0
		float2( 0.0, 0.70 ),// length:0.7 deg:90
	};

	//const float sphereLineLengths[NUM_PAIRS] = { 1.9078784, 1.42828569f } ;// Sin(invCos(length))*2
	const float invSphereLineLengths[NUM_PAIRS] = { 1.0/1.9078784, 1.0/1.42828569f } ;// 1/(Sin(invCos(length))*2)

	// centerLineLength = 2.0 ;
	const half sphereSampleWeights[NUM_PAIRS] = { 0.25f, 0.2161575f };// ここの値だけ決め方が判らなかったので適当に決めました
	const half sphereCenterWeight = 0.067685 ;

	const half2 paramRadius = (half2)g_psObject.m_localParam[0].xy ;
	const half2 paramMaxDistance = (half2)g_psObject.m_localParam[1].xy ;
	const half2 paramInvMaxDistance = (half2)g_psObject.m_localParam[2].xy ;
	const half2 paramFalloffDistance  = (half2)g_psObject.m_localParam[3].xy ;
	const half2 paramFallonDistance  = (half2)g_psObject.m_localParam[0].zw ;
	half radii[NUM_RADII] = { paramRadius.y } ;
	half maxDistances[NUM_RADII] = { paramMaxDistance.y } ;
	half invMaxDistances[NUM_RADII] = { paramInvMaxDistance.y } ;
#endif	// ifndef FAST_SSAO

	

	
	const half2 paramTempAAOffset = 0 ;

	half occlusion ;

	float invAspectRatio = g_psSystem.m_renderBuffer.y / g_psSystem.m_renderBuffer.x ;
	float2 uvCenter = inTexcoord + paramTempAAOffset ;	   
	
	half2 uvRotation = (half2)(inPixelPosition * 0.25h) ;
	
	half2 rotationSinCos = (half2)TFetch2D( inRotations, SamplerPoint, uvRotation ).rg ;
	half2x2 rotation = {
...
{-rotationSinCos.x, rotationSinCos.y}
	...
	float2 tmpUVScale = g_psScene.m_viewportSize.xy * 0.5 * g_psSystem.m_renderBuffer.zw ;
	float2 pixelRatio    = 1.0f;
#else
	float2 tmpUVScale =  1.0f;
	float2 pixelRatio = g_psObject.m_localParam[2].zw;			//ピクセルレシオ考慮
#endif
#ifndef NOT_REQUIRED_CENTRAL_CORRECTION
	float2 tmpPixelOffset = 0.5 * 0.5 * g_psSystem.m_renderBuffer.zw ;
#else
	float2 tmpPixelOffset = 0 ;
#endif
	uvCenter = uvCenter * tmpUVScale + tmpPixelOffset ;
	float zCenter = ReadDepthValue2( inLinearDepth, uvCenter, g_psScene.m_projectionParam );

	
	half zInvCenter = (half)(1.0h / zCenter);

	
	occlusion = 0.5 * sphereCenterWeight ;
	...
	for ( int i = 0 ; i < NUM_RADII; i ++ ) {
		half amount = occlusion ;
		
		half radius = radii[i] * zInvCenter ;
		
		radius = min ( radius, 0.07 );
		...
		float2	uvScale = float2( invAspectRatio, 1.0 ) * tmpUVScale * radius * pixelRatio;
		...
		for ( int j = 0; j < NUM_PAIRS ; j++ ) {
			...
			float2	sampleOffset = samplePoint.xy * uvScale.xy ;
			
			half	invLineLength = (half)invSphereLineLengths[j];

			float2 uv1 = uvCenter + sampleOffset.xy ;
			float2 uv2 = uvCenter - sampleOffset.xy ;
			float2 zSamples ; 
			zSamples.x = ReadDepthValue2( inLinearDepth, uv1, g_psScene.m_projectionParam );
			zSamples.y = ReadDepthValue2( inLinearDepth, uv2, g_psScene.m_projectionParam );

			
			half2 zDiff = half2(zCenter.xx - zSamples.xy) ;
			
			half2 occlusionContribs = (half2)saturate( ( zDiff * invLineLength.xx) + 0.5h ) ;

			
			half2 distanceMod = (half2)saturate( ( maxDistances[i] - zDiff.xy ) * invMaxDistances[i] );
			
			
			occlusionContribs = (half2)lerp(
			    lerp( 0.5f, 1.0f - occlusionContribs.yx, distanceMod.yx ),
			    occlusionContribs.xy, distanceMod.xy ) ;

			
			occlusionContribs *= sphereSampleWeights[j] ;
			
			amount += occlusionContribs.x ;
			amount += occlusionContribs.y ;
}

		
		amounts +=(half) saturate( ( amount - 0.5h ) * 2.0h );
}
	...
	occlusion = (half)saturate( amounts );  
	

	
	
	half fallon  = (half)saturate( (zCenter + paramFallonDistance.x ) * paramFallonDistance.y );
	half falloff = (half)( zCenter + paramFalloffDistance.x ) * paramFalloffDistance.y ;
	occlusion *= fallon * (half)saturate( falloff ) ;

#if 0
	outColor.rgb = half3( 1.0 - occlusion.xxx ) ;
	outColor.a = 1.0h ;
#else
	
	
	
	zCenter = ( zCenter - g_psScene.m_viewportSize.z ) / g_psScene.m_viewportSize.w ;
	outColor.rgb = packFP32( zCenter ).xyz ;
	
	
	outColor.a = (half) ( 1.0h - occlusion ) ;
#endif
...
Texture2D	NVolumetricOcclusion_volumetricOcclusion_inRotations;
Texture2D	NVolumetricOcclusion_volumetricOcclusion_inLinearDepth;
Texture2D	inLinearDepth	:TEXUNIT0 ;
Texture2D	inRotations	:TEXUNIT4 ;
#line 290 "..\Gr\Dg\shader\SSAOLineIntegrals.shdr"
void ps_main(
...
#line 307 "..\Gr\Dg\shader\SSAOLineIntegrals.shdr"
	...
	float2	NVolumetricOcclusion_volumetricOcclusion_inTexcoord ;
	float2	NVolumetricOcclusion_volumetricOcclusion_inPixelPosition ;
			half4 NVolumetricOcclusion_volumetricOcclusion_outColor ;
#line 309 "..\Gr\Dg\shader\SSAOLineIntegrals.shdr"
	NVolumetricOcclusion_volumetricOcclusion_inTexcoord = NClipToTextureCoordinate_clipToTexcoord_outTexcoord ;
#line 310 "..\Gr\Dg\shader\SSAOLineIntegrals.shdr"
	NVolumetricOcclusion_volumetricOcclusion_inPixelPosition = inVPos.xy ;
	NVolumetricOcclusion( NVolumetricOcclusion_volumetricOcclusion_inTexcoord, NVolumetricOcclusion_volumetricOcclusion_inPixelPosition, inLinearDepth, inRotations, NVolumetricOcclusion_volumetricOcclusion_outColor ) ;


#line 314 "..\Gr\Dg\shader\SSAOLineIntegrals.shdr"
	outColor = NVolumetricOcclusion_volumetricOcclusion_outColor ;
