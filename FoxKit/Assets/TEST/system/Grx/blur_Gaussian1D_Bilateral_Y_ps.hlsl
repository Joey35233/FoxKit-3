...
#line 4 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_Y.shdr"


#line 16 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"



#line 13 "..\Gr\Dg\shader\blur_Gaussian1D_X.shdr"
...
#line 14 "..\Gr\Dg\shader\blur_Gaussian1D_X.shdr"
...
#line 19 "..\Gr\Dg\shader\blur_Gaussian1D_X.shdr"


#line 33 "..\Gr\Dg\shader\blur_Gaussian1D_X.shdr"
...
#define GAUSS_NUM_SAMPLES (5)
#define GAUSS_SUM (1.0/10.0)



#line 45 "..\Gr\Dg\shader\blur_Gaussian1D_X.shdr"


#line 60 "..\Gr\Dg\shader\blur_Gaussian1D_X.shdr"




#line 63 "..\Gr\Dg\shader\blur_Gaussian1D_X.shdr"


#line 71 "..\Gr\Dg\shader\blur_Gaussian1D_X.shdr"




#line 74 "..\Gr\Dg\shader\blur_Gaussian1D_X.shdr"


#line 127 "..\Gr\Dg\shader\blur_Gaussian1D_X.shdr"




#line 153 "..\Gr\Dg\shader\blur_Gaussian1D_X.shdr"


#line 17 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"




#line 24 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"


#line 50 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"



#line 52 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"


#line 70 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"



float GetUnpackedDepth(float3 packedValue, float4 viewportSize)
{

#line 79 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"
	const float far = viewportSize.w;

	return unpackFP32(float4(packedValue.xyz, 0)) * far;
}

#define EPSILON (1.0)


#line 89 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"


#line 207 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"





#line 224 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"



#line 256 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"


#line 5 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_Y.shdr"




#line 8 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_Y.shdr"


#line 16 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_Y.shdr"



#line 91 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"
inline void NBlur1DEx(float2 inTexcoord, float2 inPixelOffset, Texture2D inImage, out half4 outColor)
{
#line 98 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"
...
#define NUM_SAMPLES (5)
	...
	float fCenterDepth;
	half fSum;
	//const float2 dxPixelOffset = 0.5 ;
	//float2 pixelSize = g_psSystem.m_renderBuffer.zw ;
	const half kernelGauss[NUM_SAMPLES] = {
		0.067, 0.242, 0.383, 0.242, 0.067 // sigma = 1.0
	};
	const int HALF = NUM_SAMPLES >> 1;

	//texcoord = inTexcoord + pixelSize * 0.5 ; 
	texcoord = inTexcoord;
#ifdef F_TARGET_WIN32
	texcoord = clamp(texcoord, 0.0, g_psObject.m_localParam[0].zw);
#endif
	centerValue = TFetch2D(inImage, SamplerPoint, texcoord).xyzw;
	occlusion = centerValue.a * kernelGauss[HALF];
	fCenterDepth = GetUnpackedDepth(centerValue.xyz, g_psScene.m_viewportSize);
	fSum = kernelGauss[HALF];

	// can't use bilinear interpolation to sample double amount of points
	// since we are doing bilateral filtering...
#ifndef CGLANG
	[unroll]
#endif // CGLANG
	for (i = 0; i < HALF; i++) {
		float j = (float)(i - HALF);
		//texcoord = inTexcoord + pixelSize * ( inUVMask * j + dxPixelOffset );
		texcoord = inTexcoord + inPixelOffset * j;
#ifdef F_TARGET_WIN32
		texcoord = clamp(texcoord, 0.0, g_psObject.m_localParam[0].zw);
#endif
		float4 value = TFetch2D(inImage, SamplerPoint, texcoord);
		float fDepth = GetUnpackedDepth(value.xyz, g_psScene.m_viewportSize);
		half fDepthDiff = (half)(fDepth - fCenterDepth);
		half fWeight = kernelGauss[i] / (abs(fDepthDiff) + 1.0h);
		fSum += fWeight;
		occlusion += fWeight * value.a;
	}
#ifndef CGLANG
	[unroll]
#endif // CGLANG
	for (i = HALF + 1; i < NUM_SAMPLES; i++) {
		float j = (float)(i - HALF);
		//texcoord = inTexcoord + pixelSize * ( inUVMask * j + dxPixelOffset ) ;
		texcoord = inTexcoord + inPixelOffset * j;
#ifdef F_TARGET_WIN32
	...
	outColor.xyz = (half3) centerValue.xyz;
	outColor.w = (half) (occlusion / fSum);
	//outColor = (half4) ( value ) ;   
#else
#define NUM_SAMPLES (5)
#define NUM_CENTER	(2)
	const half kernelGauss[NUM_SAMPLES] = {
		0.067, 0.242, 0.383, 0.242, 0.067
	};
	float2	texcoord0 = inTexcoord + inPixelOffset * (-2.0);
	float2	texcoord1 = inTexcoord + inPixelOffset * (-1.0);
	float2	texcoord2 = inTexcoord;
	float2	texcoord3 = inTexcoord + inPixelOffset * (1.0);
	float2	texcoord4 = inTexcoord + inPixelOffset * (2.0);
#ifdef F_TARGET_WIN32
	texcoord0 = clamp(texcoord0, 0.0, g_psObject.m_localParam[0].zw);
	texcoord1 = clamp(texcoord1, 0.0, g_psObject.m_localParam[0].zw);
	texcoord2 = clamp(texcoord2, 0.0, g_psObject.m_localParam[0].zw);
	texcoord3 = clamp(texcoord3, 0.0, g_psObject.m_localParam[0].zw);
	texcoord4 = clamp(texcoord4, 0.0, g_psObject.m_localParam[0].zw);
#endif
	half4 value0 = TFetch2DH(inImage, SamplerPoint, texcoord0);
	half4 value1 = TFetch2DH(inImage, SamplerPoint, texcoord1);
	half4 valueCenter = TFetch2DH(inImage, SamplerPoint, texcoord2);
	half4 value3 = TFetch2DH(inImage, SamplerPoint, texcoord3);
	half4 value4 = TFetch2DH(inImage, SamplerPoint, texcoord4);

	half	occlusion = valueCenter.a * kernelGauss[NUM_CENTER];
	float	fCenterDepth = GetUnpackedDepth(valueCenter.xyz, g_psScene.m_viewportSize);
	half	fSum = kernelGauss[NUM_CENTER];

	half4	occludes = half4(value0.a, value1.a, value3.a, value4.a);
	float4	depths;
	depths.x = GetUnpackedDepth(value0.xyz, g_psScene.m_viewportSize);
	depths.y = GetUnpackedDepth(value1.xyz, g_psScene.m_viewportSize);
	depths.z = GetUnpackedDepth(value3.xyz, g_psScene.m_viewportSize);
	depths.w = GetUnpackedDepth(value4.xyz, g_psScene.m_viewportSize);
	half4	depthDiffs = (half4)abs(depths.xyzw - fCenterDepth) + EPSILON;
	half4	gaussKernel = { kernelGauss[0],kernelGauss[1],kernelGauss[3],kernelGauss[4] };
	half4	weights = gaussKernel.xyzw / depthDiffs.xyzw;
	fSum += weights.x + weights.y + weights.z + weights.w;
	occlusion += dot(occludes, weights);

	outColor.xyz = (half3) valueCenter.xyz;
	outColor.w = (half) (occlusion / fSum);
#endif
}
...
#line 211 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"
inline void NGenerateTexcoordAndPixelOffset(float4 inUvParameter0, float2 inPixelOffset, out float2 outTexcoord, out float2 outPixelOffset)
{
#line 218 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"

	outTexcoord = inUvParameter0.xy;
#ifdef F_TARGET_XBOX360
	outTexcoord += g_psTiling.m_vposOffset.xy * g_psSystem.m_renderBuffer.zw;
#endif
	outPixelOffset = inPixelOffset * inUvParameter0.zw;
}



#line 10 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_Y.shdr"
inline void NGetUVMask(out float2 outUVMask)
{
#line 14 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_Y.shdr"

	outUVMask = float2(0, 1);
}
...
Texture2D	NBlur1DEx_blur1D_inImage;
Texture2D	inImage	:TEXUNIT0;
#line 226 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"
void ps_main(

	in	float4	inVPos	: VPOS,
	in	float4	inUvParameter0 : TEXCOORD0,
...
)
{
	...
	float2 NGetUVMask_getUVMask_outUVMask;
	NGetUVMask(NGetUVMask_getUVMask_outUVMask);


	float4	NGenerateTexcoordAndPixelOffset_generateTexcooredAndPixelOffset_inUvParameter0;
	float2	NGenerateTexcoordAndPixelOffset_generateTexcooredAndPixelOffset_inPixelOffset;
	float2 NGenerateTexcoordAndPixelOffset_generateTexcooredAndPixelOffset_outTexcoord;
	float2 NGenerateTexcoordAndPixelOffset_generateTexcooredAndPixelOffset_outPixelOffset;
#line 246 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"
	NGenerateTexcoordAndPixelOffset_generateTexcooredAndPixelOffset_inUvParameter0 = inUvParameter0;
#line 247 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"
	NGenerateTexcoordAndPixelOffset_generateTexcooredAndPixelOffset_inPixelOffset = NGetUVMask_getUVMask_outUVMask;
	NGenerateTexcoordAndPixelOffset(NGenerateTexcoordAndPixelOffset_generateTexcooredAndPixelOffset_inUvParameter0, NGenerateTexcoordAndPixelOffset_generateTexcooredAndPixelOffset_inPixelOffset, NGenerateTexcoordAndPixelOffset_generateTexcooredAndPixelOffset_outTexcoord, NGenerateTexcoordAndPixelOffset_generateTexcooredAndPixelOffset_outPixelOffset);


	float2	NBlur1DEx_blur1D_inTexcoord;
	float2	NBlur1DEx_blur1D_inPixelOffset;
	half4 NBlur1DEx_blur1D_outColor;
#line 251 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"
	NBlur1DEx_blur1D_inTexcoord = NGenerateTexcoordAndPixelOffset_generateTexcooredAndPixelOffset_outTexcoord;
#line 252 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"
	NBlur1DEx_blur1D_inPixelOffset = NGenerateTexcoordAndPixelOffset_generateTexcooredAndPixelOffset_outPixelOffset;
	NBlur1DEx(NBlur1DEx_blur1D_inTexcoord, NBlur1DEx_blur1D_inPixelOffset, inImage, NBlur1DEx_blur1D_outColor);


#line 255 "..\Gr\Dg\shader\blur_Gaussian1D_Bilateral_X.shdr"
	outColor = NBlur1DEx_blur1D_outColor;
