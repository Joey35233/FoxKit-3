...
#line 1 "..\Gr\Dg\shader\SSAOAlphaToRGB.shdr"



#line 19 "..\Gr\Dg\shader\SSAOAlphaToRGB.shdr"



#line 45 "..\Gr\Dg\shader\SSAOAlphaToRGB.shdr"




#line 75 "..\Gr\Dg\shader\SSAOAlphaToRGB.shdr"



#line 10 "..\Gr\Dg\shader\Common.shdr"
inline void NTex2D(float2 inTexCoord, Texture2D inTexture, out float4 outColor)
{
#line 16 "..\Gr\Dg\shader\Common.shdr"

	outColor = TFetch2D(inTexture, SamplerLinear, inTexCoord);
}



#line 41 "..\Gr\Dg\shader\Common.shdr"
inline void NScreenToTextureCoordinate(float2 inPixelPosition, out float2 outTexcoord)
{
#line 46 "..\Gr\Dg\shader\Common.shdr"
...
#line 21 "..\Gr\Dg\shader\SSAOAlphaToRGB.shdr"
inline void NEncodeColor(half4 inColor, out half4 outColor)
{
#line 29 "..\Gr\Dg\shader\SSAOAlphaToRGB.shdr"

#if 0
	float contrast = 2.0 * g_psObject.m_localParam[1].x - 1.0;
	float ambient = contrast > 0 ?
		saturate((inColor.a - 0.5 * contrast) / (1.0 - contrast))
		: (1.0 + contrast) * inColor.a - contrast;
	ambient *= ambient; // linearな色空間で自然な減衰を実現する為に二乗しておく
#else


	float lowThreshold = g_psObject.m_localParam[1].x;
	float invRange = g_psObject.m_localParam[1].y;
	float ambient = lerp(1.0f, saturate((inColor.a * invRange + lowThreshold)), g_psObject.m_localParam[1].w); // J - lowThreshold is 0, invRange is 1, ambient lerp is 1 (result is no change)
#endif
	outColor.rgb = (half) ambient;
	outColor.a = 1.0h;
}



Texture2D	NTex2D_sampleTexture_inTexture;
Texture2D	inSourceBuffer	:TEXUNIT0;
#line 48 "..\Gr\Dg\shader\SSAOAlphaToRGB.shdr"
void ps_main(
...
)
{
	...
	float2	NScreenToTextureCoordinate_screenToTextureCoordinate_inPixelPosition;
	float2 NScreenToTextureCoordinate_screenToTextureCoordinate_outTexcoord;
#line 65 "..\Gr\Dg\shader\SSAOAlphaToRGB.shdr"
	NScreenToTextureCoordinate_screenToTextureCoordinate_inPixelPosition = inVPos.xy;
	NScreenToTextureCoordinate(NScreenToTextureCoordinate_screenToTextureCoordinate_inPixelPosition, NScreenToTextureCoordinate_screenToTextureCoordinate_outTexcoord);


	float2	NTex2D_sampleTexture_inTexCoord;
	float4 NTex2D_sampleTexture_outColor;
#line 67 "..\Gr\Dg\shader\SSAOAlphaToRGB.shdr"
	NTex2D_sampleTexture_inTexCoord = NScreenToTextureCoordinate_screenToTextureCoordinate_outTexcoord;
	NTex2D(NTex2D_sampleTexture_inTexCoord, inSourceBuffer, NTex2D_sampleTexture_outColor);


	half4	NEncodeColor_encodeColor_inColor;
	half4 NEncodeColor_encodeColor_outColor;
#line 70 "..\Gr\Dg\shader\SSAOAlphaToRGB.shdr"
	NEncodeColor_encodeColor_inColor = (half4)NTex2D_sampleTexture_outColor;
	NEncodeColor(NEncodeColor_encodeColor_inColor, NEncodeColor_encodeColor_outColor);


#line 73 "..\Gr\Dg\shader\SSAOAlphaToRGB.shdr"
	outColor = NEncodeColor_encodeColor_outColor;
