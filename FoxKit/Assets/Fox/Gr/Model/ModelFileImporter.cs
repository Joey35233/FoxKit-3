...
//
//             reader.Seek(def.GetFeaturePositionForIndex(FeatureType.StringIds, index));
//
...
//             long rewindPosition = reader.BaseStream.Position;
//
//             reader.Seek(def.GetFeaturePositionForIndex(FeatureType.BoundingBoxes, index));
//
//             var box = new Bounds();
...
//             if (def.HasFeature(FeatureType.BoneDef))
//             {
//                 uint boneCount = def.GetFeatureCount(FeatureType.BoneDef);
//
...
//                 {
//                     reader.Seek(def.GetFeaturePositionForIndex(FeatureType.BoneDef, i));
//
//                     ushort nameIndex = reader.ReadUInt16();
//                     if (!def.ValidateFeatureIndex(FeatureType.BoneDef, FeatureType.StringIds, nameIndex))
//                         return;
...
//                     if (parentIndex > -1 && !def.ValidateFeatureIndex(FeatureType.BoneDef, FeatureType.BoneDef, (uint)parentIndex))
//                         return;
//
//                     ushort aabbIndex = reader.ReadUInt16();
//                     if (!def.ValidateFeatureIndex(FeatureType.BoneDef, FeatureType.BoundingBoxes, aabbIndex))
//                         return;
...
//             if (def.HasFeature(FeatureType.MeshHeader))
//             {
//                 uint meshGroupHeaderCount = def.GetFeatureCount(FeatureType.MeshHeader);
//
//                 meshGroups = new GameObject[meshGroupHeaderCount];
//
//                 for (uint i = 0; i < meshGroupHeaderCount; i++)
//                 {
//                     reader.Seek(def.GetFeaturePositionForIndex(FeatureType.MeshHeader, i));
//
//                     ushort nameIndex = reader.ReadUInt16();
//                     if (!def.ValidateFeatureIndex(FeatureType.MeshHeader, FeatureType.StringIds, nameIndex))
//                         return;
//                     StrCode nameHash = ReadNameForIndex(ref def, reader, nameIndex);
//
...
//
//                     short parentIndex = reader.ReadInt16();
//                     if (parentIndex > -1 && !def.ValidateFeatureIndex(FeatureType.MeshHeader, FeatureType.MeshHeader, (uint)parentIndex))
//                         return;
//
...
//             if (def.HasFeature(FeatureType.MeshDef))
//             {
//                 var meshGroupDefs = new AggregateMeshGroupDef[meshGroups.Length];
//
//                 {
//                     uint meshGroupDefCount = def.GetFeatureCount(FeatureType.MeshDef);
//
//                     // Initial loops for preallocation and input validation
//                     for (uint i = 0; i < meshGroupDefCount; i++)
//                     {
//                         reader.Seek(def.GetFeaturePositionForIndex(FeatureType.MeshDef, i));
//
//                         uint unknown = reader.ReadUInt32();
//                         Debug.Assert(unknown == 0);
//
//                         ushort headerIndex = reader.ReadUInt16();
//                         if (!def.ValidateFeatureIndex(FeatureType.MeshDef, FeatureType.MeshHeader,
//                                 headerIndex))
//                             return;
//
//                         ushort meshCount = reader.ReadUInt16();
//                         ushort meshesStartIndex = reader.ReadUInt16();
//                         if (meshCount > 0 && !def.ValidateFeatureIndex(FeatureType.MeshDef, FeatureType.MeshPacket,
...
//
//                         ushort aabbIndex = reader.ReadUInt16();
//                         if (!def.ValidateFeatureIndex(FeatureType.MeshDef, FeatureType.BoundingBoxes, aabbIndex))
//                             return;
//
//                         reader.SkipPadding(4);
//
//                         ushort iBufferSlicesGroupStartIndex = reader.ReadUInt16();
//                         if (!def.ValidateFeatureIndex(FeatureType.MeshDef, FeatureType.IBufferSlices,
...
//                     {
//                         reader.Seek(def.GetFeaturePositionForIndex(FeatureType.MeshPacket, aggregateMeshGroupDef.MeshDefIndices[j]));
//
//                         uint flags = reader.ReadUInt32();
//
//                         ushort materialInstanceIndex = reader.ReadUInt16();
//                         if (!def.ValidateFeatureIndex(FeatureType.MeshPacket, FeatureType.Material, materialInstanceIndex))
//                             return;
//
//                         ushort boneGroupIndex = reader.ReadUInt16();
//                         if (!def.ValidateFeatureIndex(FeatureType.MeshPacket, FeatureType.BoneGroups, boneGroupIndex))
//                             return;
//
//                         ushort dataLayoutDescIndex = reader.ReadUInt16();
//                         if (!def.ValidateFeatureIndex(FeatureType.MeshPacket, FeatureType.MeshDataLayoutDescs, dataLayoutDescIndex))
//                             return;
//
//                         // TODO - Inc. validation
