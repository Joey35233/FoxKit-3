...
namespace Fox.Grx
{
    public static class ColorUtils
    {
        public static Vector2 CalculateBlackBodyPrimaries(float temperature)
        {
            ...
            if (4000.0 <= temperature)
            {
                fVar3 = (2107038.0f / (temperature * temperature) -
                        3.025847e+09f / (temperature * temperature * temperature)) + 222.6347f / temperature +
                        0.24039f;
            }
            else
            {
                fVar3 = (877.6956f / temperature -
                        (2.661239e+08f / (temperature * temperature * temperature) +
                        234358.0f / (temperature * temperature))) + 0.17991f;
            }
            if (2222.0 <= temperature)
            {
                if (4000.0 <= temperature)
                {
                    fVar2 = ((fVar3 * fVar3 * fVar3 * 3.081758f - fVar3 * fVar3 * 5.873387f) + fVar3 * 3.75113f) -
                            0.3700148f;
                }
                else
                {
                    fVar2 = (fVar3 * 2.09137f - (fVar3 * fVar3 * fVar3 * 0.9549476f + fVar3 * fVar3 * 1.374186f)) -
                            0.1674887f;
                }
            }
            else
            {
                fVar2 = (fVar3 * 2.185558f - (fVar3 * fVar3 * fVar3 * 1.106381f + fVar3 * fVar3 * 1.34811f)) -
                        0.2021968f;
            }
